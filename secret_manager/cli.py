...
from secret_manager.core import ProjectManager, RemoteManager, SecretManager
from secret_manager.core.schemas import Backend, Project, Remote, SecretMode
from secret_manager.utils import logger
from secret_manager.utils.selection import select_from_list
from secret_manager.wizards.project import resolve_comparison_environments, select_environment
from secret_manager.wizards.remote import configure_aws_backend, configure_s3_key, select_or_create_remote
...
def track(secrets_file: Path, environment: str = None):
    ...
    try:
        ...
        env = SecretMode(environment) if environment else select_environment("Select the environment to track secrets:")
...
def track_remote(environment: str = None):
    ...
    try:
        ...
        # Use wizard to select environment if not provided
        env = SecretMode(environment) if environment else select_environment("Select environment to configure remote tracking:")
...
def push_secrets(
    ...
    try:
        ...
        # Use wizard to select environment if not provided
        env = SecretMode(environment) if environment else select_environment("Select environment to push secrets for:")
        ...
        if not force:
            confirm = typer.confirm(f"This will overwrite remote secrets for {env} environment. Continue?", prompt_suffix=" ", default=False)
            if not confirm:
...
@app.command("fetch")
def fetch_secrets(environment: str = typer.Argument(None, help="Environment to fetch (e.g., dev, prod)")):
    ...
    try:
        ...
        # Use wizard to select environment if not provided
        env = SecretMode(environment) if environment else select_environment("Select environment to fetch secrets for:")
...
@app.command("pull")
def pull_secrets(
    environment: str = typer.Argument(None, help="Environment to pull (e.g., dev, prod)"),
    force: bool = typer.Option(False, "--force", "-f", help="Skip confirmation prompt"),
):
    ...
    try:
        ...
        # Use wizard to select environment if not provided
        env = SecretMode(environment) if environment else select_environment("Select environment to pull secrets for:")
        ...
        # Ask for confirmation unless force is specified
        if not force:
            confirm = typer.confirm(f"This will overwrite local secrets for {env.value} environment. Continue?", prompt_suffix=" ", default=False)
            if not confirm:
                logger.info("Operation cancelled")
                return 0
...
def unset_remote(environment: str = None):
    ...
    try:
        ...
        # Use wizard to select environment if not provided
        env = SecretMode(environment) if environment else select_environment("Select environment to remove remote tracking:")
...
def remote_show(name: str = None):
    ...
    if not name:
        ...
        name = select_from_list(message="Select a remote to view details:", choices=[r.name for r in remotes])
