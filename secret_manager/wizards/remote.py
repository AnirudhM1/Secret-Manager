...
from secret_manager.utils.aws_profiles import get_aws_profile_credentials, read_aws_profiles
from secret_manager.utils.selection import select_from_list


def configure_aws_backend() -> AWSConfig:
    """Interactive wizard to configure AWS backend"""

    # Ask whether to use AWS profile or manual credentials
    credential_method = select_from_list(message="How would you like to provide AWS credentials?", choices=["Use AWS Profile", "Enter manually"])
    ...
    return AWSConfig(AWS_ACCESS_KEY_ID=aws_access_key_id, AWS_SECRET_ACCESS_KEY=aws_secret_access_key, AWS_REGION=aws_region)
...
def select_or_create_remote() -> tuple[str, bool]:
    ...
    selection = select_from_list(message="Select a remote or create a new one:", choices=choices)
