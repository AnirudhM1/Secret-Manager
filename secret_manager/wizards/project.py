...
def select_environment(prompt_message: str = None) -> SecretMode:
    ...
    env = select_from_list(message=message, choices=["local", "dev", "prod"])
    return SecretMode(env)


def resolve_comparison_environments(source: str = None, target: str = None) -> tuple[SecretMode, SecretMode]:
    ...
    if source:
        ...
        target = select_from_list(message="Select target environment:", choices=target_choices, default=default_target)
    ...
    if target:
        ...
        source = select_from_list(message="Select source environment:", choices=source_choices, default=default_source)
        source_mode = SecretMode(source)
        return source_mode, target_mode

    # Case 4: Neither provided, select both
    source = select_from_list(message="Select source environment:", choices=available_envs, default="local")
